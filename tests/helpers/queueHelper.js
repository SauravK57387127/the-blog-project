...
export async function waitForJob(queue, jobId, timeout = 10000) {
  ...
  return new Promise((resolve, reject) => {
      ...
      try {
        ...
        if (job) {
          ...
          if (state === 'completed') {
            ...
            return resolve({ status: 'completed', result: returnvalue });
          }
          if (state === 'failed') {
            await queueEvents.close();
            return resolve({ status: 'failed', error: job.failedReason });
          }
        }
      } catch (err) {
        // Ignore
    ...
    queueEvents.on('completed', async ({ jobId: completedId, returnvalue }) => {
