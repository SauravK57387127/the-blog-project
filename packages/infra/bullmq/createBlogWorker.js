 ...
 * @returns {Worker}
 */
export function createBlogWorker(
    redisConnection,
    Blog,
...
async function handlePublishBlog(job, Blog, jobLogger) {
    ...
    try {
        ...
        if (blog.status !== 'scheduled') {
            jobLogger.warn('Blog status changed', {
                blogId,
        ...
        if (error.name === 'ValidationError') {
            throw new FatalError('Blog validation failed', {
                blogId,
                error: error.message,
            });
        }
    ...
    try {
        ...
        if (error.name === 'MongoNetworkError') {
            throw new RetryableError(
                'Database connection failed during reconciliation',
            );
        }
        throw error;
    }
