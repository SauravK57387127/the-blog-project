...
BlogAnalyticsSchema.methods.recalculate = async function() {
  ...
  this.avgReadTime = readViews.length > 0
  ...
  this.totalBookmarks = await Bookmark.countDocuments({ blogId: this.blogId });
  
  this.readRate = this.totalViews > 0
    ? (this.totalReads / this.totalViews) * 100
    : 0;
  
  this.completionRate = this.totalReads > 0
    ? (this.totalCompletions / this.totalReads) * 100
    : 0;
  
  this.engagementRate = this.totalViews > 0
    ? ((this.totalLikes + this.totalComments + this.totalBookmarks) / this.totalViews) * 100
    : 0;
