...
import { useWritingStreak, useSidebarStats } from '@/hooks/api/admin/useAdminHome';
...
export default function WritingStreak() {
  ...
  const period = view === 'monthly' ? 'month' : view === 'weekly' ? 'week' : 'year';

  const { data: streakData,  isLoading: streakLoading  } = useWritingStreak(period);
  const { data: sidebarData, isLoading: sidebarLoading } = useSidebarStats(period);
