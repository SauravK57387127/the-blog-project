...
export default function CategoryBreakdown({ data, isLoading }) {
    ...
    const filtered = data
        .filter((d) => MAIN_CATS.includes(d.category))
        .slice(0, 3);
    if (!filtered.length) return null;

    const totalViews = filtered.reduce(
        (sum, d) => sum + parseNum(d.totalViews),
        0,
    );
                ...
                {filtered.map((item) => {
                    ...
                    const sharePct =
                        totalViews > 0
                            ...
                            : 0;
                    const meta = LABELS[item.category] ?? {
                        short: `[ ${item.category} ]`,
                        label: item.category,
                    };
                    return (
                            ...
                            <div>
                                ...
                                <div className="h-[3px] bg-muted w-full">
                                    <div
                                        ...
                                        style={{
                                            width: `${item.percentage ?? sharePct}%`,
                                        }}
                                    />
                                </div>
                            </div>
