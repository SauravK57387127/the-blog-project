...
export default function StaleDrafts({ drafts, onEdit }) {
  const maxDays = Math.max(...drafts.map(d =>
    d.updatedAt ? getDaysStale(d.updatedAt) : 0
  ), 1);

  const sortedDrafts = [...drafts].sort((a, b) => {
    const dA = a.updatedAt ? getDaysStale(a.updatedAt) : parseInt(a.lastUpdated) || 0;
    const dB = b.updatedAt ? getDaysStale(b.updatedAt) : parseInt(b.lastUpdated) || 0;
    return dB - dA;
  });
...
const barClass =
  ratio < 0.25 ? 'bg-accent/25' :
  ratio < 0.5  ? 'bg-accent/60' :
  ratio < 0.75 ? 'bg-destructive/40' :
                 'bg-destructive/80'; 
