...
const demoRelatedBlogs = [
    {
        ...
        excerpt:
            "Deep dive into React Server Components and how they're changing the way we build applications.",
        coverImage:
    ...
    },
    {
        ...
        excerpt:
            "Explore the best AI tools that are transforming how developers work today.",
        coverImage:
    ...
    },
    {
        ...
        excerpt:
            "Practical TypeScript patterns that will make your code safer and more maintainable.",
        coverImage:
...
function BigCard({ blog }) {
    return (
        <Link
            href={`/blog/${blog.slug}`}
            className="group block relative overflow-hidden bg-muted h-full min-h-[280px]"
        >
            {blog.coverImage && (
                <img
                    src={blog.coverImage}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
                ...
                <h3
                    className={`font-sans font-bold text-white text-lg leading-snug group-hover:text-accent ${DESIGN_CONSTANTS.transitions.fast}`}
                >
                    {blog.title}
                </h3>
            </div>
            ...
            <div
                className={`absolute bottom-0 left-0 w-0 h-[3px] bg-accent group-hover:w-full ${DESIGN_CONSTANTS.transitions.smooth}`}
            />
        </Link>
    );
}
...
function TextCard({ blog }) {
    return (
        <Link
            href={`/blog/${blog.slug}`}
            className="group flex gap-3 p-4 h-full items-center"
        >
            {/* Text */}
...
function HorizontalCard({ blog }) {
    return (
        <Link
            href={`/blog/${blog.slug}`}
            className="group flex gap-4 border-l-4 border-border hover:border-accent pl-4 py-2 transition-all duration-200"
        >
            {/* Text */}
