...
export default function HeroSection({  
  ...
  let coverImage = "https://images.unsplash.com/photo-1461749280684-dccba630e2f6?w=1200&h=600&fit=crop"
  
  if (isLoading) {
    return (
      <section>
        <div className={`${DESIGN_CONSTANTS.containers.standard} mx-auto px-4 sm:px-6 lg:px-8 ${DESIGN_CONSTANTS.spacing.section}`}>
  ...
  if (!blog) {
    return (
      <section>
        <div className={`${DESIGN_CONSTANTS.containers.standard} mx-auto px-4 sm:px-6 lg:px-8 py-16`}>
          <p className="text-center text-muted-foreground">No featured blog available</p>
        </div>
      </section>
    );
  }
    ...
    <div className={`${DESIGN_CONSTANTS.containers.standard} mx-auto px-4 sm:px-6 lg:px-8 ${DESIGN_CONSTANTS.spacing.section}`}>
      <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        ...
        >
          ...
          {/* Title */}
          <h1 className={`${DESIGN_CONSTANTS.typography.heroTitle}`}>
            {blog.title}
          </h1>

          {/* Excerpt — lifted from muted to foreground/75 */}
          <p className="text-lg text-foreground/75 leading-relaxed font-reading">
            {blog.excerpt || blog.content?.substring(0, 180)}...
          </p>

          {/* CTA — underline draw animation, no bouncing arrow */}
        ...
        >
          {blog.coverImage || coverImage ? (
            <img
              src={blog.coverImage || coverImage}
              alt={blog.title}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-accent/20 to-primary/10">
              ...
              <p className="text-sm text-muted-foreground">Cover image coming soon</p>
            </div>
          )}
        </div>
      </div>
