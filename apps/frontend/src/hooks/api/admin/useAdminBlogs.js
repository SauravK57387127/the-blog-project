...
export function usePublishedBlogs({ status, category, page = 1, limit = 20 } = {}) {
  return useQuery({
    queryKey: keys.published({ status, category, page }),
    queryFn: () => adminBlogsService.getPublished({ status, category, page, limit }),
    staleTime: 60 * 1000,
...
export function usePublishedBlogsCounts(categoryFilter) {
  return useQuery({
    ...
    queryFn: () => adminBlogsService.getPublished({
      category: categoryFilter !== 'all' ? categoryFilter : undefined,
      limit: 500,
    }),
    staleTime: 60 * 1000,
    select: (r) => ({
      published: (r?.data?.blogs ?? []).filter(b => b.status === 'published').length,
      scheduled: (r?.data?.blogs ?? []).filter(b => b.status === 'scheduled').length,
    }),
  });
}
