...
const demoNotifications = [
    {
        ...
        blogTitle: "The Art of Clean Code",
        blogSlug: "art-of-clean-code",
        commentPreview:
            "Great point! I'd also add that naming conventions matter just as much.",
    },
    {
        _id: "n2",
        type: "like_comment",
        ...
        blogSlug: "getting-started-modern-web-dev",
        commentPreview:
            'Your comment: "This was exactly what I needed to understand..."',
    },
    {
        _id: "n3",
        type: "reply",
        ...
        blogTitle: "DSA Problem Solving: My Daily Approach",
        blogSlug: "dsa-daily-approach",
        commentPreview:
            "Have you tried the two-pointer approach for this type of problem?",
    },
];
...
const demoLikedBlogs = [
    ...
    },
    {
        ...
        coverImage:
            "https://images.unsplash.com/photo-1506126613408-eca07ce68773?w=120&h=80&fit=crop",
        tags: ["Life & Growth"],
        readingTime: 4,
        publishedAt: new Date(
...
const demoBookmarkedBlogs = [
    {
        ...
        coverImage:
            "https://images.unsplash.com/photo-1516116216624-53e697fedbea?w=120&h=80&fit=crop",
...
const demoComments = [
    {
        _id: "c1",
        content:
            "This was exactly what I needed. The Server Components section cleared up so many doubts I had been carrying for weeks.",
        blogTitle: "Getting Started with Modern Web Development",
        blogSlug: "getting-started-modern-web-dev",
        createdAt: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
        replyCount: 2,
    },
    {
        _id: "c2",
        content:
            "Great point about naming conventions! I'd also add that consistent file structure matters just as much as the code itself.",
        blogTitle: "The Art of Clean Code",
        blogSlug: "art-of-clean-code",
        createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        replyCount: 1,
...
const demoHistory = [
    {
        ...
        coverImage:
            "https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?w=120&h=80&fit=crop",
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                <h3
                    className={`font-sans font-semibold text-sm leading-snug line-clamp-2 group-hover:text-accent ${DESIGN_CONSTANTS.transitions.fast}`}
                >
                    {blog.title}
                </h3>
        ...
        >
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs text-muted-foreground min-w-0">
                    ...
                    <span
                        className={`font-medium text-foreground truncate group-hover:text-accent ${DESIGN_CONSTANTS.transitions.fast}`}
                    >
                        {comment.blogTitle}
                    </span>
                </div>
...
export default function UserProfile() {
    ...
    return (
        <div className="w-full h-screen overflow-hidden">
            <div
                className={`${DESIGN_CONSTANTS.containers.standard} mx-auto px-4 sm:px-6 lg:px-8 py-10`}
            >
                {/* Profile Header */}
                    ...
                    {activeTab === "notifications" && (
                        <TabWindow>
                            ...
                            {notifications.length === 0 ? (
                                ...
                                <>
                                    {notifications
                                        .slice(0, visibleCounts.notifications)
                                        .map((n) => (
                                            <NotificationItem
                                                key={n._id}
                                                notification={n}
                                    ...
                                    <LoadMore
                                        onLoad={() => loadMore("notifications")}
                                        remaining={
                                            notifications.length -
                                            visibleCounts.notifications
                                        }
                                    />
                                </>
                            )}
                        ...
                        <TabWindow>
                            {demoLikedBlogs.length === 0 ? (
                                ...
                                <>
                                    ...
                                    <LoadMore
                                        onLoad={() => loadMore("liked")}
                                        remaining={
                                            demoLikedBlogs.length -
                                            visibleCounts.liked
                                        }
                                    />
                                </>
                    ...
                    {activeTab === "bookmarked" && (
                        <TabWindow>
                            {demoBookmarkedBlogs.length === 0 ? (
                                ...
                                <>
                                    {demoBookmarkedBlogs
                                        .slice(0, visibleCounts.bookmarked)
                                        .map((blog) => (
                                            <BlogRow
                                                key={blog._id}
                                                blog={blog}
                                    ...
                                    <LoadMore
                                        onLoad={() => loadMore("bookmarked")}
                                        remaining={
                                            demoBookmarkedBlogs.length -
                                            visibleCounts.bookmarked
                                        }
                                    />
                                </>
                            )}
                    ...
                    {activeTab === "comments" && (
                        <TabWindow>
                            {demoComments.length === 0 ? (
                                <EmptyState
                                    icon={MessageCircle}
                                    message="You haven't commented yet."
                                />
                            ) : (
                                <>
                                    {demoComments
                                        .slice(0, visibleCounts.comments)
                                        .map((comment) => (
                                            <CommentItem
                                                key={comment._id}
                                                comment={comment}
                                    ...
                                    <LoadMore
                                        onLoad={() => loadMore("comments")}
                                        remaining={
                                            demoComments.length -
                                            visibleCounts.comments
                                        }
                                    />
                                </>
                            )}
                    ...
                    {activeTab === "history" && (
                        <TabWindow>
                            {demoHistory.length === 0 ? (
                                ...
                                <>
                                    {demoHistory
                                        .slice(0, visibleCounts.history)
                                        .map((blog) => (
                                            <BlogRow
                                                key={blog._id}
                                                blog={blog}
                                                meta={`Read ${formatTimeAgo(blog.readAt)}`}
                                                onNavigate={navigate}
                                            />
                                        ))}
                                    <LoadMore
                                        onLoad={() => loadMore("history")}
                                        remaining={
                                            demoHistory.length -
                                            visibleCounts.history
                                        }
                                    />
                                </>
                            )}
