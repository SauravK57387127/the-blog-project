...
function useDrafts(page = 1) {
    return useQuery({
        ...
        queryFn: () =>
            apiClient.get(API_ENDPOINTS.ADMIN.BLOGS.DRAFTS, {
...
function useDeleteDraft() {
    ...
    return useMutation({
        mutationFn: (id) =>
            apiClient.delete(API_ENDPOINTS.ADMIN.BLOGS.DELETE(id)),
        onSuccess: () =>
...
export default function DraftsPageComponent() {
    ...
    const filtered = accDrafts.filter(
        (d) =>
            !debouncedQuery ||
                ...
                onSuccess: () => {
                    ...
                    toast.error("Draft deleted", {
                        ...
                        action: {
                            ...
                            onClick: () =>
                                toast.info(
                                    "Undo not available — draft permanently deleted",
                                ),
                        },
                    });
                },
        ...
        <AdminLayout_New>
            <div className="space-y-8">
                ...
                {isLoading && page === 1 ? (
                    ...
                    <EmptyState
                        onCreateNew={() => router.push("/admin/blogs/new")}
                    />
                ) : filtered.length === 0 ? (
