...
const demoLastDraft = {
    ...
    excerpt:
        "A comprehensive guide to React Server Components, their benefits, and how to use them effectively.",
...
function QuickActions({ onNewPost, onViewBlog, onDrafts }) {
    const actions = [
        {
            label: "New Post",
            descriptor: "+ start writing",
            icon: Plus,
            onClick: onNewPost,
        },
        {
            label: "View Blog",
            descriptor: "→ live site",
            icon: Globe,
            onClick: onViewBlog,
        },
        {
            label: "Drafts",
            descriptor: "edit existing",
            icon: FileText,
            onClick: onDrafts,
        },
    ];
                ...
                return (
                    ...
                    >
                        ...
                        <div>
                            ...
                            <p
                                className={`text-xs font-mono text-muted-foreground group-hover:text-background/60 ${DESIGN_CONSTANTS.transitions.fast}`}
                            >
                                {action.descriptor}
                            </p>
                        </div>
...
function WritingStreak() {
    ...
    const weeklyData = (() => {
        ...
        for (let i = 0; i < last84.length; i += 7) {
            ...
            const totalActivity = week.reduce(
                (sum, d) =>
                    sum +
                    (d.published || 0) +
                    (d.scheduled || 0) +
            ...
            const weekLabel = new Date(week[0].date).toLocaleDateString(
                "en-US",
        ...
        for (let m = 11; m >= 0; m--) {
            ...
            const monthDays = streakData.filter((day) => {
                ...
                return (
                    ...
                    dayDate.getFullYear() === d.getFullYear()
                );
            });
            const published = monthDays.reduce(
                (sum, d) => sum + (d.published || 0),
                0,
            );
            const scheduled = monthDays.reduce(
                (sum, d) => sum + (d.scheduled || 0),
                0,
            );
            const drafted = monthDays.reduce(
                (sum, d) => sum + (d.drafted || 0),
                0,
            );
            months.push({
                label: monthName,
                ...
                scheduled,
                drafted,
                activeDays: monthDays.filter((d) => getActivityLevel(d) > 0)
                    .length,
            });
        }
        return months;
                            ...
                            <div className="space-y-2">
                                {topDays.map((day, i) => (
                                    ...
                                    >
                                        ...
                                        <div
                                            className={`w-2 h-2 flex-shrink-0 ${getLevelClass(getActivityLevel(day))}`}
                                        />
                                ...
                                {[
                                    {
                                        ...
                                        value: weeklyData.reduce(
                                            (best, w) =>
                                                w.total > best.total ? w : best,
                                            weeklyData[0],
                                        )?.label,
                                    },
                                    {
                                        label: "Avg per week",
                                        value: `${(weeklyData.reduce((sum, w) => sum + w.total, 0) / weeklyData.length).toFixed(1)} posts`,
                                    },
                                    {
                                        label: "Active weeks",
                                        value: `${weeklyData.filter((w) => w.total > 0).length} / ${weeklyData.length}`,
                                    },
                                ].map((stat) => (
                                    <div key={stat.label}>
                                ...
                                {[
                                    {
                                        ...
                                        value: `${yearlyData.reduce((sum, m) => sum + m.published, 0)} posts`,
                                    },
                                    {
                                        label: "Total scheduled",
                                        value: `${yearlyData.reduce((sum, m) => sum + m.scheduled, 0)} posts`,
                                    },
                                    {
                                        label: "Total drafted",
                                        value: `${yearlyData.reduce((sum, m) => sum + m.drafted, 0)} drafts`,
                                    },
                                    {
                                        label: "Best month",
