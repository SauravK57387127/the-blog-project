...
import {
    usePublishedBlogs,
    usePublishedBlogsCounts,
    useDeleteBlog,
    usePublishNow,
    useToggleEditorsPick,
...
function StatusBadge({ status }) {
    return (
        <span
            className={`inline-flex items-center text-xs font-mono font-medium px-2 py-0.5 border ${
                status === "published"
...
function BlogRow({
    ...
    return (
        <div
            className={`group flex items-center gap-4 py-4 border-b border-border hover:bg-muted/20 ${DESIGN_CONSTANTS.transitions.fast}`}
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                    <p
                        className={`font-sans font-semibold text-sm line-clamp-1 group-hover:text-accent ${DESIGN_CONSTANTS.transitions.fast}`}
                    >
                        {blog.title}
                    </p>
                    {isPick && (
                    ...
                    <button
                        ...
                        title={
                            isPick
                                ...
                                : "Add to editor's choice"
                        }
                    >
                        <Star
                            className={`h-4 w-4 ${isPick ? "fill-accent" : ""}`}
                        />
                    </button>
                )}
                <button
...
function EmptyState({ statusFilter, categoryFilter, onReset }) {
    return (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
            <p className="text-4xl font-mono font-bold text-muted-foreground/10">
                0
            </p>
...
export default function AdminBlogsPage() {
    ...
    const { data, isLoading, isFetching } = usePublishedBlogs({
        status:
            statusFilter === "all" || statusFilter === "editors-choice"
                ? undefined
                : statusFilter,
    ...
    const { mutate: togglePick, isPending: isTogglingPick } =
        useToggleEditorsPick();
    ...
    const blogs = accBlogs
        ...
        .filter((blog) => {
            if (statusFilter === "editors-choice")
                return blog.editorsPick?.isEditorsPick === true;
            if (statusFilter === "published")
                return (
                    blog.status === "published" &&
                    !blog.editorsPick?.isEditorsPick
                );
            ...
            deleteBlog(blog._id, {
                onSuccess: () => {
                    ...
                    toast.success("Blog deleted", {
                        action: {
                            label: "Undo",
                            onClick: () => toast.info("Undo not available"),
                        },
                        duration: 5000,
                    });
                },
    ...
    const handleEditorsPick = useCallback(
        (blog) => {
            const isPick = blog.editorsPick?.isEditorsPick ?? false;
            if (isPick) {
                togglePick(
                    { blogId: blog._id, isEditorsPick: false, annotation: "" },
                    {
                        onSuccess: () => {
                            setAccBlogs((prev) =>
                                prev.map((b) =>
    ...
    const handleSavePick = useCallback(() => {
        if (!pickModal) return;
        togglePick(
            { blogId: pickModal.blog._id, isEditorsPick: true, annotation },
            {
                onSuccess: () => {
                    setAccBlogs((prev) =>
                        prev.map((b) =>
    ...
    return (
        <AdminLayout_New>
            ...
            <Dialog
                open={!!pickModal}
                onOpenChange={(open) => !open && setPickModal(null)}
            >
                <DialogContent>
                    <DialogHeader>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <p className="text-sm font-sans font-semibold line-clamp-2">
                                {pickModal?.blog.title}
                            </p>
                        </div>
                        <div>
                            ...
                            <p className="text-[10px] font-mono text-muted-foreground mt-1">
                                {annotation.length}/150 chars
                            </p>
                        </div>
                        <p className="text-xs font-reading text-muted-foreground/60 italic">
                            If 4 picks already exist, the oldest will be
                            replaced.
                        </p>
