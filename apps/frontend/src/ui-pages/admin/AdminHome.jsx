...
const WritingStreak     = dynamic(() => import('@/components/admin/WritingStreak'),     { loading: () => <div className="h-32 bg-muted animate-pulse rounded" /> });
const ScheduledTimeline = dynamic(() => import('@/components/admin/ScheduledTimeline'), { loading: () => <div className="h-48 bg-muted animate-pulse rounded" /> });
const StaleDrafts       = dynamic(() => import('@/components/admin/StaleDrafts'),       { loading: () => <div className="h-48 bg-muted animate-pulse rounded" /> });
...
export default function AdminHomePage() {
  ...
  const handleContinue  = useCallback((id) => router.push(`/admin/drafts/${id}`), [router]);
  const handleEdit      = useCallback((id) => router.push(`/admin/drafts/${id}`), [router]);
  const handleNewPost   = useCallback(() => router.push('/admin/blogs/new'), [router]);
  const handleDrafts    = useCallback(() => router.push('/admin/drafts'), [router]);
  ...
  return (
    <AdminLayout_New>
      <div className="space-y-12">
        ...
        {/* 1. Quick Actions */}
        <section>
          <QuickActions onNewPost={handleNewPost} onDrafts={handleDrafts} />
        </section>
        ...
        <section>
          ...
          {draftLoading
            ...
            : <PickUpDraft draft={recentDraft} onContinue={handleContinue} />
          }
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          {staleLoading
            ? <div className="h-48 bg-muted animate-pulse rounded" />
            : <StaleDrafts drafts={staleDrafts ?? []} onEdit={handleEdit} />
          }
        </section>
