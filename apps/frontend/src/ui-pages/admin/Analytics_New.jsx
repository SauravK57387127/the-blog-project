...
const topPosts = [
  { _id: "1", title: "Getting Started with React Server Components", slug: "getting-started-with-react-server-components", views: 5420, category: "tech-deep-dive", publishedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString() },
  { _id: "2", title: "The Self-Taught Developer's Journey", slug: "self-taught-developer-journey", views: 4850, category: "career-and-learnings", publishedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString() },
  { _id: "3", title: "Building Better Habits as a Developer", slug: "building-better-habits", views: 3240, category: "life-and-growth", publishedAt: new Date(Date.now() - 15 * 24 * 60 * 60 * 1000).toISOString() },
  { _id: "4", title: "Advanced TypeScript Patterns", slug: "advanced-typescript-patterns", views: 2890, category: "tech-deep-dive", publishedAt: new Date(Date.now() - 20 * 24 * 60 * 60 * 1000).toISOString() },
  { _id: "5", title: "Why I Started Meditating Daily", slug: "why-i-started-meditating", views: 2140, category: "life-and-growth", publishedAt: new Date(Date.now() - 25 * 24 * 60 * 60 * 1000).toISOString() },
];

const categoryAnalytics = [
  { category: "tech-deep-dive", label: "Tech Deep Dive", totalPosts: 18, totalViews: 18540 },
  { category: "life-and-growth", label: "Life & Growth", totalPosts: 12, totalViews: 12340 },
  { category: "career-and-learnings", label: "Career & Learnings", totalPosts: 8, totalViews: 8450 },
];
...
function StatTile({ title, value, sub, highlight = false }) {
  return (
    <div className={`p-6 border-2 ${highlight ? "border-foreground" : "border-border"} ${DESIGN_CONSTANTS.transitions.smooth}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-6">
          <div className="text-right">
            ...
            <p className="text-sm font-mono font-bold">{formatNumber(totalViews)}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">Daily avg</p>
            <p className="text-sm font-mono font-bold">{formatNumber(avgViews)}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">Trend</p>
            <p className={`text-sm font-mono font-bold ${trend ? "text-accent" : "text-destructive/70"}`}>
        ...
        {hoveredIndex !== null && (
          <div
            ...
            style={{
              ...
              transform: hoveredIndex > data.length * 0.7 ? "translateX(-100%)" : "translateX(8px)",
            }}
          >
            <div className="bg-foreground text-background text-[10px] font-mono px-2 py-1 whitespace-nowrap">
              {data[hoveredIndex].date} · {formatNumber(data[hoveredIndex].views)} views
            </div>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-2 border-foreground divide-y-2 md:divide-y-0 md:divide-x-2 divide-foreground">
        {data.map((item) => {
          const avgViewsPerPost = Math.floor(item.totalViews / item.totalPosts);
          const sharePct = Math.round((item.totalViews / totalViews) * 100);
          ...
          return (
            <div key={item.category} className="p-6 space-y-4">
              <div>
                ...
                <h3 className="font-sans font-bold text-base mt-1">{item.label}</h3>
              </div>

              <div className="space-y-3">
                <div>
                  <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-widest">Total views</p>
                  <p className="text-2xl font-mono font-bold">{formatNumber(item.totalViews)}</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-mono font-bold">{formatNumber(avgViewsPerPost)}</p>
                  </div>
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-[10px] font-mono font-bold">{sharePct}%</span>
                </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-2 border-foreground divide-y-2 md:divide-y-0 md:divide-x-2 divide-foreground">
        ...
        <div className="p-6 border-l-4 border-l-accent">
          ...
          <p className="text-4xl font-mono font-bold mt-3">{formatNumber(bestDay.views)}</p>
        ...
        <div className="p-6 border-l-4 border-l-foreground/30">
          ...
          <p className="text-4xl font-mono font-bold mt-3">{formatNumber(bestMonth.views)}</p>
    ...
    <AdminLayout_New>
      <div className="space-y-12">
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatTile title="Total Views" value={formatNumber(analyticsOverview.totalViews)} sub="all-time" highlight />
          <StatTile title="Published Posts" value={analyticsOverview.totalBlogs} sub="articles live" />
          <StatTile title="Avg Read Time" value={`${analyticsOverview.avgReadTime}m`} sub="per article" />
          <StatTile title="Subscribers" value={analyticsOverview.subscribers} sub={`+${analyticsOverview.subscriberGrowth} this month`} />
        </section>
