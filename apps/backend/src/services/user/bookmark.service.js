...
export default {
  ...
  toggleBookmark: async ({ userId, blogId }) => {
    try {
      ...
      logger.error('Toggle bookmark failed', { error: error.message, userId, blogId });
      return {
        success: false,
  ...
  removeBookmark: async ({ userId, blogId }) => {
    try {
      ...
      logger.error('Remove bookmark failed', { error: error.message, userId, blogId });
      return {
        success: false,
...
getMyBookmarks: async ({ userId, page, limit }) => {
  try {
    ...
    logger.info('User bookmarks fetched', { userId, count: bookmarks.length });
    ...
    logger.error('Get my bookmarks failed', { error: error.message, userId });
    return {
      success: false,
