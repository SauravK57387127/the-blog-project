...
export default {
  ...
  handleUserCreated: async (clerkUser) => {
    try {
      ...
      if (!email) {
        logger.error('User created without email', { clerkUserId: clerkUser.id });
        return;
      }
  ...
  return;
      ...
      logger.info('User created in MongoDB', { clerkUserId: clerkUser.id, mongoId: user._id });
      ...
      if (subscriber) {
        ...
        logger.info('Linked subscriber to user', { email, userId: user._id });
      } else {
  ...
  handleUserUpdated: async (clerkUser) => {
    try {
      ...
      if (!user) {
        logger.warn('User.updated for non-existent user, creating', { clerkUserId: clerkUser.id });
      ...
      logger.info('User updated in MongoDB', { clerkUserId: clerkUser.id });
    } catch (error) {
  ...
  handleUserDeleted: async (clerkUser) => {
    try {
      ...
      if (!user) {
        logger.warn('User.deleted for non-existent user', { clerkUserId: clerkUser.id });
        return;
      }
