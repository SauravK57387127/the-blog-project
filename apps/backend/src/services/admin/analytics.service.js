...
export default {
    ...
    getWritingStreak: async ({ period = 'month' }) => {
        try {
            ...
            logger.info('Writing streak data fetched', {
                period,
            ...
            logger.error('Get writing streak failed', {
                error: error.message,
                period,
            });
            return {
    ...
    getSidebarStats: async ({ period = 'year' }) => {
        try {
            ...
            const longestStreak28 = await calculateLongestStreak(
                last28Start,
                now,
            );
            ...
            if (period === 'year') {
                ...
                const [totalPublished, totalScheduled, totalDrafts] =
                    await Promise.all([
                        Blog.countDocuments({
                            status: 'published',
            ...
            logger.error('Get sidebar stats failed', {
                error: error.message,
                period,
            });
            return {
    ...
    getScheduledUpcoming: async ({ limit = 5 }) => {
        try {
            ...
            logger.info('Scheduled upcoming fetched', {
            ...
            logger.error('Get scheduled upcoming failed', {
                error: error.message,
            });
            return {
                success: false,
