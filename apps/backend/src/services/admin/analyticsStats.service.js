...
export default {
    ...
    getViewsLast30Days: async () => {
        try {
            ...
            const recentViews = result
                .slice(15)
                .reduce((sum, day) => sum + day.views, 0);
            const previousViews = result
                .slice(0, 15)
                .reduce((sum, day) => sum + day.views, 0);
            const trend =
                recentViews > previousViews ? 'growing' : 'decreasing';

            logger.info('Views last 30 days fetched', {
                totalViews,
                dailyAverage,
            });
            ...
            logger.error('Get views last 30 days failed', {
                error: error.message,
            });
            return {
                success: false,
    ...
    getTopPerformingPosts: async ({ limit = 5 }) => {
        try {
            const topAnalytics = await BlogAnalytics.find({
                views180d: { $gt: 0 },
            })
                .sort({ views180d: -1 })
                .limit(limit)
            ...
            logger.info('Top performing posts fetched', {
            ...
            logger.error('Get top performing posts failed', {
                error: error.message,
            });
            return {
                success: false,
    ...
    getCategoryBreakdown: async () => {
        try {
            ...
            logger.error('Get category breakdown failed', {
                error: error.message,
            });
            return {
                success: false,
    ...
    getPersonalBest: async () => {
        try {
            ...
            const allViews = await BlogView.find()
                .select('blogId viewedAt')
                .lean();

            // Group by day
            const dayViewsMap = new Map();
            allViews.forEach((view) => {
                const dateStr = new Date(view.viewedAt)
                    .toISOString()
                    .split('T')[0];
            ...
            if (bestDay.views > 0) {
                const blog = await Blog.findById(bestDay.blogId)
                    .select('title')
                    .lean();
                bestDayData = {
            ...
            allViews.forEach((view) => {
                ...
                monthViewsMap.set(
                    monthKey,
                    (monthViewsMap.get(monthKey) || 0) + 1,
                );
            });
            ...
            allBlogs.forEach((blog) => {
                ...
                monthPostsMap.set(
                    monthKey,
                    (monthPostsMap.get(monthKey) || 0) + 1,
                );
            });
            ...
            for (const [month, views] of monthViewsMap.entries()) {
                if (views > bestMonth.views) {
                    bestMonth = {
                        month,
                        views,
                        posts: monthPostsMap.get(month) || 0,
                    };
                }
            }
