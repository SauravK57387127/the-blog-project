...
export default {
        ...
        try {
            ...
            const [blogs, totalCount] = await Promise.all([
                Blog.find(query)
                    ...
                    .limit(limit)
                    .select(
                        'title slug draftSlug coverImage status category publishedAt scheduledAt editorsPick',
                    )
                    .lean(),
            ...
            logger.error('Get published blogs failed', {
                error: error.message,
            });
            return {
                success: false,
        ...
        try {
            ...
            if (blog.status === 'scheduled' && blogQueue) {
                ...
                try {
                    ...
                    if (job) {
                        await job.remove();
                        logger.info('Cancelled scheduled publish job', {
                            jobId,
                        });
                    }
                } catch (error) {
                    logger.warn('Could not cancel job', {
                        jobId,
                        error: error.message,
                    });
                }
            ...
            logger.error('Delete blog failed', {
                error: error.message,
                blogId,
            });
            return {
    ...
    listBlogs: async ({ status, search, page, limit }) => {
        try {
            ...
            const [blogs, totalCount] = await Promise.all([
                Blog.find(query)
                    .select(
                        'title slug status coverImage tags category publishedAt scheduledAt createdAt updatedAt',
                    )
                    .sort({ updatedAt: -1 })
                    .skip(skip)
                    .limit(limit)
            ...
            logger.info('Admin blogs listed', {
                status,
                count: blogs.length,
                totalCount,
            });
    ...
    getBlogByDraftSlug: async (draftSlug) => {
        try {
            ...
            logger.error('Get blog by draft slug failed', {
                error: error.message,
                draftSlug,
            });
            return {
    ...
    updateBlog: async (blogId, updates) => {
        try {
            ...
            logger.error('Update blog failed', {
                error: error.message,
                blogId,
            });
            return {
...
readingTime: calculateReadingTime(blog.content),
            ...
            logger.error('Publish blog failed', {
                error: error.message,
                blogId,
            });
            return {
    ...
    scheduleBlog: async (blogId, scheduledAt, { blogQueue }) => {
        ...
        try {
            ...
            logger.error('Schedule blog failed', {
                error: error.message,
                blogId,
            });
            return {
    ...
    unpublishBlog: async (blogId, { blogQueue }) => {
        try {
            ...
            logger.error('Unpublish blog failed', {
                error: error.message,
                blogId,
            });
            return {
    ...
    toggleEditorsPick: async (blogId, { isEditorsPick, annotation }) => {
        try {
            ...
            if (!isEditorsPick) {
                ...
                return {
                    success: true,
                    message: 'Removed from editors choice',
                    data: null,
                };
            }
            ...
            if (currentPicks.length >= 4) {
                const oldest = currentPicks.sort(
                    (a, b) =>
                        (a.editorsPick?.pickOrder ?? 0) -
                        (b.editorsPick?.pickOrder ?? 0),
                )[0];
                await Blog.findByIdAndUpdate(
                    oldest._id,
                    {
                ...
                logger.info('Oldest editors pick removed to make room', {
                    removed: oldest._id,
                });
            }
            ...
            return {
                success: true,
                message: 'Added to editors choice',
                data: null,
            };
        } catch (error) {
            logger.error('Toggle editors choice failed', {
                error: error.message,
                blogId,
            });
            return {
                success: false,
                message: 'Failed to toggle editors choice',
                data: null,
            };
        }
    },
