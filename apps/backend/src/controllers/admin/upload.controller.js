...
export default {
  ...
  uploadCoverImage: asyncHandler(async (req, res) => {
    ...
    const result = await UploadService.uploadCoverImage({ file, base64, url, draftSlug });
  ...
  uploadContentImage: asyncHandler(async (req, res) => {
    ...
    const result = await UploadService.uploadContentImage({ file, draftSlug });
  ...
  deleteCoverImage: asyncHandler(async (req, res) => {
    ...
    if (!publicId) {
      return sendResponse({
        res, statusCode: 400, success: false,
        message: 'publicId required', data: null,
      });
    }
